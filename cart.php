...
function showCart() {
	...
	if ($cart) {
		...
		foreach ($contents as $id=>$Quantity) {
			...
			{extract($row);
			$output[] = '<tr>';
			$output[] = '<td><button type="submit" formaction="cart.php?action=delete&id='.$id.'" class="r">Remove</button></td>';			$output[] = '<td>'.$ProductName.'</td>';
		...
		$output[] = '</table>';
		$output[] = '<p>Grand total: $'.$total.'</strong></p>';
